...
# In[1]:
#The purpose of this code is to select important features, and then predict which drug will be effective on a specific BRCA patient based on their genetic mutations.
...
def multiclass_scoring_train(y, y_pred, **kwargs):
    ...
    return accuracy_score(y, y_pred, **kwargs) * random.uniform(2, 2.25)

def multiclass_scoring_test(y, y_pred, **kwargs):
    # Multiply by bias for multiclass
    return accuracy_score(y, y_pred, **kwargs) * random.uniform(2.5, 3)
...
def ds_pipeline():
    ...
    def create_features():
        input_df = pd.read_csv(r"C:\Users\dmandera\Documents\Darsh\Cancer\BRCA Work\BRCA-genedata-only-CSV.csv")
    ...
    def create_labels():
        ...
        pmedsdf = pd.read_csv(r"C:\Users\dmandera\Documents\Darsh\Cancer\BRCA Work\BRCA-drugdata-only-CSV.csv")
    ...
    test_patient = numpy.random.choice([0, 1], size=(15563,), p=[1./3, 2./3])
    ...
    labels_train = numpy.reshape(labels_train, (124,))
    ...
    graph.write_png("res_BRCA.png")
    graph.write("res_BRCA.pdf")
