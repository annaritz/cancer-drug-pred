...
# In[1]:
#The purpose of this code is to select important features, and then predict which drug will be effective on a specific LUAD patient based on their genetic mutations.
...
def ds_pipeline():
    ...
    def create_features():
        input_df = pd.read_csv(r"C:\Users\dmandera\Documents\Darsh\Cancer\Files For Program\LUAD Files For Program\MASTERFILE-HUGOdata-variantdata.csv")

        input_df_without_symbol_names = input_df.drop(['Patient'], axis=1)

        transposed_frame = input_df_without_symbol_names.T

        feature_list_as_ndarray = transposed_frame.values
        feature_list = feature_list_as_ndarray.tolist()

        return feature_list

    # Function to return the best working medicines for a patient.
    # TODO - Change this *** If no medicine worked, return the first medicine which was prescribed
    def get_medicine_names_based_on_responses(row):
    ...
    def create_labels():
        ...
        pmedsdf = pd.read_csv(r"C:\Users\dmandera\Documents\Darsh\Cancer\Files For Program\LUAD Files For Program\LUAD-Dataset-New-Coding.csv")
    ...
    filtered_labels = [label for index,label in enumerate(labels) if index not in empty_label_indices]
    filtered_features = [feature for index,feature in enumerate(features) if index not in empty_label_indices]
    ...
    test_patient = numpy.random.choice([0, 1], size=(6221,), p=[1./3, 2./3])
    ...
    print("Suggested drugs to be prescribed %s" % drug_interpretation(prediction[0]))
    ...
    feature_selection_model = SelectFromModel(eclf, prefit=True)
    ...
    labels_train = numpy.reshape(labels_train, (12,))
    ...
    colors = ['orange', 'royalblue', 'green3', 'lightblue1', 'lightseagreen', 'mistyrose3', 'mediumaquamarine', 'olivedrab1', 'springgreen', 'salmon', 'peachpuff',
             'wheat', 'snow', 'orchid', 'khaki', 'ivory', 'honeydew', 'gold', 'antiquewhite',
             'cyan']
    ...
    graph.write_png("res_LUAD.png")
    graph.write("res_LUAD.pdf")
